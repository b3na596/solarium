...
bool CMasternodeConfig::read(boost::filesystem::path path) {
    ...
    for(std::string line; std::getline(streamConfig, line); )
    {
        ...
        if (!(iss >> alias >> ip >> privKey >> txHash >> outputIndex >> donation)) {
            ...
            if (!address.IsValid()) {
                LogPrintf("Invalid SLRC address in masternode.conf line: %s\n", line.c_str());
                streamConfig.close();
                return false;
            }
        }
        ...
        if(Params().NetworkID() == CChainParams::MAIN){
            if(CService(ip).GetPort() != 4848) {
                LogPrintf("Invalid port detected in masternode.conf: %s (must be 4848 for mainnet)\n", line.c_str());
                streamConfig.close();
                return false;
            }
        } else if(CService(ip).GetPort() == 4848) {
            LogPrintf("Invalid port detected in masternode.conf: %s (4848 must be only on mainnet)\n", line.c_str());
            streamConfig.close();
            return false;
        }

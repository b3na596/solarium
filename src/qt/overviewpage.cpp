...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeSolariumAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeSolariumAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeSolariumAmount = strAnonymizeSolariumAmount.remove(strAnonymizeSolariumAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSolariumAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeSolariumAmount*COIN) nMaxToAnonymize = nAnonymizeSolariumAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeSolariumAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeSolariumAmount));
        strAnonymizeSolariumAmount = strAnonymizeSolariumAmount.remove(strAnonymizeSolariumAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSolariumAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {

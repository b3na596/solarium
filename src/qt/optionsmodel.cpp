...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeSolariumAmount"))
        settings.setValue("nAnonymizeSolariumAmount", 1000);
    nAnonymizeSolariumAmount = settings.value("nAnonymizeSolariumAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeSolariumAmount"))
        SoftSetArg("-anonymizeSolariumamount", settings.value("nAnonymizeSolariumAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeSolariumAmount:
            return QVariant(nAnonymizeSolariumAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeSolariumAmount:
            nAnonymizeSolariumAmount = value.toInt();
            settings.setValue("nAnonymizeSolariumAmount", nAnonymizeSolariumAmount);
            emit AnonymizeSolariumAmountChanged(nAnonymizeSolariumAmount);
            break;
        default:
            break;
        }

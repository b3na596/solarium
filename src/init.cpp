...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Solarium.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Solariumd.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 4848)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeSolariumamount=<n> " + _("Keep N Solarium anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Solarium is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Solarium is probably already running."), strDataDir));
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Solarium") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Solarium to complete") << "\n";
    ...
    nAnonymizeSolariumAmount = GetArg("-anonymizeSolariumamount", 0);
    if(nAnonymizeSolariumAmount > 999999) nAnonymizeSolariumAmount = 999999;
    if(nAnonymizeSolariumAmount < 2) nAnonymizeSolariumAmount = 2;
    ...
    LogPrintf("Anonymize Solarium Amount %d\n", nAnonymizeSolariumAmount);
